...
function init() {
  ...
  d3.json("Womens_Clothing_E-Commerce_Reviews.json").then((data) => {
    var sampleNames = data.ClothingID;
...
function buildMetadata(sample) {
  d3.json("samples.Womens_Clothing_E-Commerce_Reviews.json").then((data) => {
...
function buildCharts(sample) {
  ...
  d3.json("Womens_Clothing_E-Commerce_Reviews.json").then((data) => {
    ...
    var barLayout = {
      title: "Top 10 Product by Category", 
      margin: {
        l:75,
        r:75,
        b:75,
...
// Bar and Bubble charts
  // 1. Create the trace for the bubble chart.
    ...
    var bubbleLayout = {
      title: 'Average Review by Product Type',
      showledgend: true,
      xaxis: {title: "OTU ID"},
      height: 500,
      width: 1000,
  ...
  // 3. Create a variable that holds the washing frequency
    ...
    var gaugeData = [{
      title: {
        text: "<b>Average Review Rating</b><br>lorum ipsulum"
      },
      value: washfreq,
      type: "indicator",
      mode: "gauge+number",
